// Scan-like fiber barrier test matching ILGPU's actual pattern.
// No mid-loop verification. Read-compute-write-barrier cycles.
// Usage: node run-scan-test.mjs [workers] [threadsPerWorker] [phases] [rounds]
...
if (!isMainThread) {
    ...
    const wasmBytes = readFileSync(new URL('./scan-barrier-test.wasm', import.meta.url));
    const thisFile = fileURLToPath(import.meta.url);

    console.log(`Scan Barrier Test: ${WORKERS} workers, ${THREADS_PER_WORKER} threads/worker, ${TOTAL_THREADS} total, ${PHASES} phases, ${ROUNDS} rounds`);
