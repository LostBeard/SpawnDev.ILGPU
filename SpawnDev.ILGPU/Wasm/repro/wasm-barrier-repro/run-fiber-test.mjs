// Fiber-model barrier test matching ILGPU's phase dispatcher.
// Usage: node run-fiber-test.mjs [workers] [threadsPerWorker] [phases] [rounds]
...
const MEM_SIZE = 32 + TOTAL_THREADS * 4 + 16;
...
if (!isMainThread) {
    ...
    const wasmBytes = readFileSync(new URL('./fiber-barrier-test.wasm', import.meta.url));
    const thisFile = fileURLToPath(import.meta.url);

    console.log(`Fiber Barrier Test: ${WORKERS} workers, ${THREADS_PER_WORKER} threads/worker, ${TOTAL_THREADS} total threads, ${PHASES} phases, ${ROUNDS} rounds`);
    ...
    for (const mode of ['spin', 'wait32']) {
        ...
        for (let round = 0; round < ROUNDS; round++) {
            ...
            const violIdx = 24 >> 2; // violation counter at byte 24
