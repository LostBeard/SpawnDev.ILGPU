...
## One-store-per-thread contract — multi-store / scatter (selection-gate only so far; compile guard is a HARD open problem)

Transform Feedback captures **exactly ONE output record per vertex (thread)**: store-slot `s` of vertex `v` lands at output index `v*storeCount+s` (`glWorker.js` readback). So a kernel where one thread writes **more than one element of the same output buffer** at offsets that do NOT match that positional layout, scatters - TF silently keeps only what positional readback expects and drops/relocates the rest. This is gather-only by construction (same reason RadixSort scatter needs the host render-to-texture path above). It silently produced garbage for ML kernels TWICE (TurboQuant `NormalizeImpl` looped writing all `d` elements/thread; FWHT butterfly wrote `data[i]` + `data[i+halfSize]`/thread) before root-cause - both fixed ML-side.

**Selection gate (shipped):** `AcceleratorRequirements.RequiresScatterStores` (WebGL=false) lets a consumer filter WebGL up front for kernels that need in-kernel scatter / >1 output element per thread.

**Compile-time fail-loud guard (NOT shipped - first attempt backed out 2026-06-13):** a blunt IR pre-scan false-positived on 40 legitimate WebGL kernels. The two naive signals do NOT work:
- `_outputStoreCount > 1` is a NORMAL, correct pattern - the positional multi-store mechanism handles `GpuMatrix4x4` (16 floats/thread), `Idct16Row`, `ManyViews`, IntBuffer pack, etc. AND `_outputStoreCount` (distinct *textual* LEA exprs) over-counts a single element written through branchy SSA (`DoubleInfinity`, `DivisionByZero`).
- "store has a loop-variant offset" false-positives on the **ubiquitous grid-stride loop** idiom (`for i=GlobalIndex.X; i<N; i+=GridStrideLoopStride`) - e.g. `RadixSortKernel2` stores `output[pos]` (scatter pos) inside a grid-stride loop and is CORRECT.

A correct guard must distinguish "writes own slot / positional `v*K+s` / grid-stride-covered element" (valid) from "one thread writes K arbitrary/own-relative elements" (broken) - i.e. verify each store offset is `vertexIndex*storeCount + constSlot` (or `vertexIndex` for single-store), accounting for grid-stride structure, branchy single-element SSA, and the scatter-path/compile-but-don't-dispatch kernels. That is real affine + loop-structure analysis at the codegen level (likely where positional varyings are allocated), not an IR pre-scan. OPEN.
...
**Decode (f16->f32) is exact** (f16 is a strict subset of f32). **Encode (f32->f16) is IEEE round-to-nearest-even** (`_f32_to_f16`, 4.14.0+): RNE mantissa rounding + proper subnormal rounding + overflow->Inf, bit-exact to numpy.float16 / PyTorch / CUDA / OpenCL and the managed `HalfConversion`. The WGSL, GLSL, and Wasm emulation paths all match this same RNE behavior, so emulated WebGL / WebGPU / Wasm agree byte-for-byte with each other and with native CUDA/OpenCL. (Before 4.14.0 they TRUNCATED toward zero and flushed all subnormals to signed zero - a divergence from numpy + CUDA/OpenCL; fixed + verified by `DemoConsole -- bf16-f16-oracle` + the PMT `Half_FloatToHalf_RoundToNearestEven` cross-backend gate.) Inf/NaN propagate via mantissa preservation.
