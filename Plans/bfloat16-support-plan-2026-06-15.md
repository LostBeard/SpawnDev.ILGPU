...
- **Full parity (local.3):** radix-sort keys (`Interop.FloatAsInt(BFloat16)` + `Ascending/DescendingBFloat16`; per-backend `FloatAsIntCast`), struct fields (CUDA `EmitIOLoad`/`EmitIOStore` + WGSL body-struct classification; validated via `RadixSortPair<bf16,int>`), IR const-fold (`ArithmeticOperations`/`CompareOperations` `.tt` + `Convert.cs`), `AcceleratorRequirements.RequiresBFloat16`. WebGL = unpacked-f32 scatter; **WebGPU = NATIVE packed bf16 radix** (the earlier "naga/Dawn mis-compiles the descending kernel → f32-widen workaround" diagnosis was DISPROVEN in 4.13.0-local.4: the real bug was the packed sub-word `view.Length` emitting a bare `arrayLength()` = atomic<u32> WORD count instead of element count (× elements-per-word); fixed, and the f32-widen workaround REMOVED — WebGPU sorts native packed bf16 keys, Dawn `dump_shaders` confirmed Tint preserves the transform). *Remaining (deferred, separate issue): the all-constant compare-ternary `Int1` predicate-fold gap is pre-existing + not bf16-specific.*
