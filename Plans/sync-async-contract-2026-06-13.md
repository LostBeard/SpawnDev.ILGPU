...
> **FINALIZED ADDENDUM (2026-06-13, Captain-approved).** `FlushAsync` (on `Accelerator` + `AcceleratorStream` + every backend override) and the redundant `*Async` scan/sort builders (`ScanAsync`/`RadixSortAsync`/`*PairsAsync` delegates, `CreateScanAsync`/`CreateRadixSortAsync`/`CreateRadixSortPairsAsync`, `WrapSyncScan`/`WrapSyncRadixSort`/`WrapSyncRadixSortPairs`, `CreateWebGPUMultiPassScanAsync`/`CreateWasmHybridScanAsync`/`CreateMultiPassRadixSortAsync`) were **DELETED** after this analysis. Rationale: **submit is fire-and-forget and SYNCHRONOUS on every backend** — WebGPU `encoder.finish()`+`queue.submit()` are sync JS calls, WebGL/Wasm submit is a no-op, and even a P2P backend's submit is a synchronous `dataChannel.send()` enqueue (P2P uses its own `DispatchAsync` API, not the stream `Flush`). So there is no backend where sync `Flush()` throws, `FlushAsync` was never needed, and the `*Async` builders (which existed only to `await FlushAsync()` between passes under a wrong "browser scan needs async" assumption) were dead weight — the SYNC builders work on all 6 backends. The genuinely-async ops are only WAIT (`SynchronizeAsync`) and READ-RESULT (readback, scalar `ReduceAsync`). The mentions of `FlushAsync` / `*Async` builders below are the as-designed history; the shipped 4.12.0 surface has only sync `Flush()` + the async wait/read-result methods.
