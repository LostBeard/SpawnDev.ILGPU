...
**4.15.1 (newest):** **Wasm bugfix - device-local managed arrays now compile correctly.** A kernel that uses a managed local array (`var acc = new float[N];` indexed in a loop) produced silently-wrong results on the **Wasm** backend (byte-exact on the other 5); the Wasm backend now runs the `LowerArrays` IR pass and no longer aliases a local array onto shared memory. (Known follow-up: a `new T[N>32]` written/read by a **runtime** index still reads 0 on Wasm - use `LocalMemory.Allocate<T>(N)` there; tracked, GPU backends unaffected.) Full detail: [CHANGELOG.md](CHANGELOG.md).
