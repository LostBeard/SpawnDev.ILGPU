...
- **Later phases:** Wasm struct-field/`FloatAsInt` parity (the `ArrayView<BFloat16>` load/store path is complete; bf16-struct-field copies + the `FloatAsInt`/`IntAsFloat` radix-sort-key Wasm sites are the remaining f16-parity gap), capability flags (`Capabilities.BFloat16`/`BFloat16Native`, `RequiresBFloat16`), and const-fold.
