...
## 4.13.0 (unreleased) - BFloat16 (bfloat16) Phases 0-3a: core type (CPU) + WebGPU + WebGL + Wasm + OpenCL codegen (5 of 6 backends)
...
- **Later phases:** native **CUDA (PTX)** bf16 - the hardest backend: PTX has no native bf16 *arithmetic* on Ada (only `cvt.*.bf16`), so it needs the "hold bf16 in f32 registers, convert at load/store" model rather than a clean mirror of the other backends (native `__nv_bfloat16` `cvt` on sm_80+ + a shift fallback for older). Plus the Wasm struct-field/FloatAsInt parity, capability flags (`Capabilities.BFloat16`/`BFloat16Native`, `RequiresBFloat16`), and const-fold. bf16 kernels skip cleanly on CUDA for now.
