...
- **Read-only Float16 sub-word params are demoted to `array<u32>` + plain indexed load** (`WGSLKernelFunctionGenerator`). A whole-module pre-scan (`ScanForStoredSubWordParams`, mirroring the existing atomic/body-struct-output scans) walks the kernel and every helper it calls, mapping helper params back to kernel params through the call chain, and records which sub-word params are ever the target of a `Store`/`GenericAtomic`/`AtomicCAS`. A Float16 param that is **never written**, gets its **own direct binding** (not coalesced, not a body-struct field), and is **not passed by pointer to a helper** is emitted as a plain `array<u32>` binding read with a plain `param[word]` load instead of `atomicLoad(&param[word])`. The value is identical (atomicLoad of a never-atomically-written buffer == a plain load); only the serialization is removed.
- **Access mode is unchanged (`read_write`).** The demotion flips only the element type + load instruction, so the WebGPU buffer-aliasing validation surface (which is about `read` vs `read_write` overlap, not atomic-ness) is untouched - algorithm kernels that bind disjoint SubViews of one temp buffer are unaffected. Params that *are* written keep `array<atomic<u32>>` + the atomicAnd/atomicOr RMW; a kernel that reads one f16 weight and writes another f16 buffer gets both, correctly.
- **Scope:** Float16 only (the conv/GEMM/attention weight case) for this cut; bf16/fp8/fp4/int8 read-only sub-word buffers keep the atomic path and are a structurally-identical fast-follow.
- **Tests:** `BackendTestBase.Float16ReadOnlyWeight` (3 new): a read-only weight window reduction (the demoted path), two read-only f16 inputs in one kernel (multiple demotions), and the mixed read-only-weight + f16-output separation (weight plain, output atomic) - all vs the CPU float oracle on every backend. Device-free WGSL generation (`ShaderCompiler`) + real-Dawn compilation both verified.
- **Gate:** full PMT sweep green across all six backends (4082 passed / 0 failed / 282 capability-skips).
