...
## 4.13.0 (unreleased) - BFloat16 (bfloat16) Phase 0: core type + IR primitive (CPU reference path)
...
- **Transpiling-backend codegen (WebGPU -> WebGL/Wasm, CUDA/OpenCL) is Phase 1+** and not yet emitted; bf16 kernels skip on those lanes for now. Capability flags (`Capabilities.BFloat16`/`BFloat16Native`, `RequiresBFloat16`) and const-fold land with the first GPU phase that consumes them.
