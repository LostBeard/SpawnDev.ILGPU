...
namespace SpawnDev.ILGPU.Demo.UnitTests
{
    ...
    public class WasmTests : BackendTestBase
    {
        ...
        // Wasm SIMD128 Stage-3a SCATTER (indexed store) numerical gate (2026-06-20). `o[idx[i]] = v` is a
        // scatter: the 4 lanes write 4 unrelated addresses. wasm SIMD has no scatter, so it is emulated as
        // 4× (extract index lane → extract value lane → scalar store). Within a by-4 group, lanes store in
        // thread order (0→3), matching the scalar path, so colliding indices resolve identically; the test
        // uses a PERMUTATION index (unique) to keep the result unambiguous across groups. Asserts kernel_simd
        // emitted and the scatter is EXACTLY equal to scalar and a CPU reference (f32 + i32).
        [TestMethod(Timeout = 120000)]
        public async Task Wasm_Simd128_ScatterFloatMatchesScalarAndReference()
        {
            const int N = 1003; // 1003 = 17*59; 7 is coprime ⇒ (7i+3)%N is a permutation
            var a = new float[N]; for (int i = 0; i < N; i++) a[i] = MathF.Sin(i * 0.02f) * 30f + i;
            var idx = new int[N]; for (int i = 0; i < N; i++) idx[i] = (i * 7 + 3) % N;
            var reference = new float[N]; for (int i = 0; i < N; i++) reference[idx[i]] = a[i] * 2f + 1f;

            var scalar = await RunSimdScatterFloat(a, idx, N, forceSimd: false, requireSimdEmit: false);
            var simd = await RunSimdScatterFloat(a, idx, N, forceSimd: true, requireSimdEmit: true);
            AssertExactF(scalar, reference, simd, "scatter-float");
        }

        [TestMethod(Timeout = 120000)]
        public async Task Wasm_Simd128_ScatterIntMatchesScalarAndReference()
        {
            const int N = 1003;
            var a = new int[N]; for (int i = 0; i < N; i++) a[i] = (i % 91) - 45;
            var idx = new int[N]; for (int i = 0; i < N; i++) idx[i] = (i * 11 + 5) % N; // 11 coprime to 1003
            var reference = new int[N]; for (int i = 0; i < N; i++) reference[idx[i]] = a[i] * 3 - 7;

            var scalar = await RunSimdScatterInt(a, idx, N, forceSimd: false, requireSimdEmit: false);
            var simd = await RunSimdScatterInt(a, idx, N, forceSimd: true, requireSimdEmit: true);
            int mismV = 0, firstV = -1, mismS = 0;
            for (int i = 0; i < N; i++) { if (simd[i] != reference[i]) { if (mismV == 0) firstV = i; mismV++; } if (scalar[i] != reference[i]) mismS++; }
            if (mismS > 0) throw new Exception($"Wasm SCALAR scatter-int != reference: {mismS}/{N} (baseline scalar wrong).");
            if (mismV > 0) throw new Exception($"Wasm SIMD scatter-int (per-lane store) != reference: {mismV}/{N}, first@{firstV} got={simd[firstV]} exp={reference[firstV]}.");
        }

        private static void Wasm_Simd_ScatterFloatKernel(Index1D i, ArrayView<float> a, ArrayView<int> idx, ArrayView<float> o)
            => o[idx[i]] = a[i] * 2f + 1f;
        private static void Wasm_Simd_ScatterIntKernel(Index1D i, ArrayView<int> a, ArrayView<int> idx, ArrayView<int> o)
            => o[idx[i]] = a[i] * 3 - 7;

        private static async Task<float[]> RunSimdScatterFloat(float[] a, int[] idx, int N, bool forceSimd, bool requireSimdEmit)
        {
            bool savedScalar = WasmBackend.ForceScalar, savedSimd = WasmBackend.ForceSimd;
            WasmBackend.ForceScalar = !forceSimd; WasmBackend.ForceSimd = forceSimd;
            try
            {
                ...
                WasmBackend.VerboseLogging = false; WasmBackend.LastWasmBinary = null;
                using var acc = await ctx.CreateWasmAcceleratorAsync();
                var k = acc.LoadAutoGroupedStreamKernel<Index1D, ArrayView<float>, ArrayView<int>, ArrayView<float>>(Wasm_Simd_ScatterFloatKernel);
                if (requireSimdEmit && (WasmBackend.LastWasmBinary == null || !ContainsExportName(WasmBackend.LastWasmBinary, "kernel_simd")))
                    throw new Exception("ForceSimd compile did NOT emit a kernel_simd export for the float scatter.");
                using var aBuf = acc.Allocate1D(a); using var iBuf = acc.Allocate1D(idx); using var oBuf = acc.Allocate1D<float>(new float[N]);
                k((Index1D)N, aBuf.View, iBuf.View, oBuf.View); await acc.SynchronizeAsync();
                return await oBuf.CopyToHostAsync<float>();
            }
            finally { WasmBackend.ForceScalar = savedScalar; WasmBackend.ForceSimd = savedSimd; }
        }

        private static async Task<int[]> RunSimdScatterInt(int[] a, int[] idx, int N, bool forceSimd, bool requireSimdEmit)
        {
            bool savedScalar = WasmBackend.ForceScalar, savedSimd = WasmBackend.ForceSimd;
            WasmBackend.ForceScalar = !forceSimd; WasmBackend.ForceSimd = forceSimd;
            try
            {
                ...
                WasmBackend.VerboseLogging = false; WasmBackend.LastWasmBinary = null;
                using var acc = await ctx.CreateWasmAcceleratorAsync();
                var k = acc.LoadAutoGroupedStreamKernel<Index1D, ArrayView<int>, ArrayView<int>, ArrayView<int>>(Wasm_Simd_ScatterIntKernel);
                if (requireSimdEmit && (WasmBackend.LastWasmBinary == null || !ContainsExportName(WasmBackend.LastWasmBinary, "kernel_simd")))
                    throw new Exception("ForceSimd compile did NOT emit a kernel_simd export for the int scatter.");
                using var aBuf = acc.Allocate1D(a); using var iBuf = acc.Allocate1D(idx); using var oBuf = acc.Allocate1D<int>(new int[N]);
                k((Index1D)N, aBuf.View, iBuf.View, oBuf.View); await acc.SynchronizeAsync();
                return await oBuf.CopyToHostAsync<int>();
            }
            finally { WasmBackend.ForceScalar = savedScalar; WasmBackend.ForceSimd = savedSimd; }
        ...
        // Wasm SIMD128 INTERACTION hardening (2026-06-20). The SIMD shapes (loop / diamond) and value
        // kinds (gather / select / arith) were built+gated separately; these verify they COMPOSE in one
        // kernel — a classic latent-bug surface. Each asserts kernel_simd is emitted (the composition
        // actually vectorized) and the v128 dispatch is BIT-EXACT to scalar and a CPU reference.

        // Diamond whose THEN side is a gather: o[i] = a[i] > 0 ? src[idx[i]] : a[i]*2.
        [TestMethod(Timeout = 120000)]
        public async Task Wasm_Simd128_DiamondWithGatherMatchesScalar()
        {
            const int N = 1003, M = 256;
            var a = new float[N]; for (int i = 0; i < N; i++) a[i] = MathF.Sin(i * 0.04f) * 20f;
            var src = new float[M]; for (int j = 0; j < M; j++) src[j] = j * 1.5f - 50f;
            var idx = new int[N]; for (int i = 0; i < N; i++) idx[i] = (i * 7) % M;
            var reference = new float[N]; for (int i = 0; i < N; i++) reference[i] = a[i] > 0f ? src[idx[i]] : a[i] * 2f;

            var scalar = await RunDiamondGather(a, src, idx, N, false, false);
            var simd = await RunDiamondGather(a, src, idx, N, true, true);
            AssertExactF(scalar, reference, simd, "diamond+gather");
        }

        // NOTE: a loop whose body contains a data-dependent SELECT is intentionally NOT gated here. Whether
        // it vectorizes depends on ILGPU if-converting the in-loop ternary to a single-block-body Predicate
        // (then it is the canonical 4-block loop my loop path handles) vs leaving it a branch (multi-block
        // body → loop path correctly BAILS to scalar). That if-conversion is NOT guaranteed (observed both
        // ways across runs), so such a kernel's *vectorization* is nondeterministic — but its *output* is
        // always correct (vectorized or scalar-fallback). A divergent-loop-body that always vectorizes is
        // general Stage-3b (active-lane masks through the loop), not yet built. So no requireSimdEmit gate.

        private static void Wasm_Simd_DiamondGatherKernel(Index1D i, ArrayView<float> a, ArrayView<float> src, ArrayView<int> idx, ArrayView<float> o)
            => o[i] = a[i] > 0f ? src[idx[i]] : a[i] * 2f;

        private static async Task<float[]> RunDiamondGather(float[] a, float[] src, int[] idx, int N, bool forceSimd, bool requireSimdEmit)
        {
            ...
            try
            {
                ...
                WasmBackend.VerboseLogging = false; WasmBackend.LastWasmBinary = null;
                using var acc = await ctx.CreateWasmAcceleratorAsync();
                var k = acc.LoadAutoGroupedStreamKernel<Index1D, ArrayView<float>, ArrayView<float>, ArrayView<int>, ArrayView<float>>(Wasm_Simd_DiamondGatherKernel);
                if (requireSimdEmit && (WasmBackend.LastWasmBinary == null || !ContainsExportName(WasmBackend.LastWasmBinary, "kernel_simd")))
                    throw new Exception("ForceSimd did NOT emit kernel_simd for diamond+gather (composition not vectorized).");
                using var aB = acc.Allocate1D(a); using var sB = acc.Allocate1D(src); using var iB = acc.Allocate1D(idx); using var oB = acc.Allocate1D<float>(N);
                k((Index1D)N, aB.View, sB.View, iB.View, oB.View); await acc.SynchronizeAsync();
                return await oB.CopyToHostAsync<float>();
            }
            finally { WasmBackend.ForceScalar = ss; WasmBackend.ForceSimd = sm; }
        }

        // Wasm SIMD128 Stage-3a f64x2 (DOUBLE precision, 2-lane double-pumped) numerical gate (2026-06-20).
        // f64 is half the lanes of f32, so each value is two v128s (lanes 0,1 + 2,3) and each op runs twice.
        // Exercises f64x2 mul/add/sub/div/sqrt/max/min/abs/neg. IEEE f64 ops are bit-identical across scalar
        // f64.* , v128 f64x2.* and host C# double, so SIMD == scalar == reference EXACTLY. Asserts kernel_simd
        // emitted. N=1003 hits the scalar tail (which the by-4 dispatch also uses for the count%4 remainder).
        [TestMethod(Timeout = 120000)]
        public async Task Wasm_Simd128_Float64MatchesScalarAndReference()
        {
            const int N = 1003;
            var a = new double[N]; var b = new double[N];
            for (int i = 0; i < N; i++) { a[i] = (i * 0.013) - 6.0; b[i] = Math.Sin(i * 0.021) * 1.5; }
            var reference = new double[N];
            for (int i = 0; i < N; i++) reference[i] = F64RefBody(a[i], b[i]);

            var scalar = await RunSimdF64(a, b, N, forceSimd: false, requireSimdEmit: false);
            var simd = await RunSimdF64(a, b, N, forceSimd: true, requireSimdEmit: true);
            int mismV = 0, firstV = -1, mismS = 0;
            for (int i = 0; i < N; i++)
            {
            ...
            if (mismS > 0) throw new Exception($"Wasm SCALAR f64 != reference: {mismS}/{N} (baseline scalar wrong).");
            if (mismV > 0) throw new Exception($"Wasm SIMD f64x2 (double-pumped) != scalar: {mismV}/{N}, first@{firstV} simd={simd[firstV]} scalar={scalar[firstV]}.");
        }

        private static double F64RefBody(double x, double y)
        {
            double r = x * 0.5 + y;
            r = r - y * 2.0;
            r = r / 1.25;
        ...
        private static void Wasm_Simd_F64Kernel(Index1D i, ArrayView<double> a, ArrayView<double> b, ArrayView<double> o)
        {
            double x = a[i]; double y = b[i];
            double r = x * 0.5 + y;
            r = r - y * 2.0;
            r = r / 1.25;
            r = global::ILGPU.Algorithms.XMath.Sqrt(r * r + 4.0);
            r = global::ILGPU.Algorithms.XMath.Max(r, -3.0);
            r = global::ILGPU.Algorithms.XMath.Min(r, 1000.0);
            r = global::ILGPU.Algorithms.XMath.Abs(r);
            o[i] = -r;
        }

        private static async Task<double[]> RunSimdF64(double[] a, double[] b, int N, bool forceSimd, bool requireSimdEmit)
        {
            bool ss = WasmBackend.ForceScalar, sm = WasmBackend.ForceSimd;
            WasmBackend.ForceScalar = !forceSimd; WasmBackend.ForceSimd = forceSimd;
            try
            {
                ...
                WasmBackend.VerboseLogging = false; WasmBackend.LastWasmBinary = null;
                using var acc = await ctx.CreateWasmAcceleratorAsync();
                var k = acc.LoadAutoGroupedStreamKernel<Index1D, ArrayView<double>, ArrayView<double>, ArrayView<double>>(Wasm_Simd_F64Kernel);
                if (requireSimdEmit && (WasmBackend.LastWasmBinary == null || !ContainsExportName(WasmBackend.LastWasmBinary, "kernel_simd")))
                    throw new Exception("ForceSimd compile did NOT emit a kernel_simd export for the f64 kernel.");
        ...
        // wasm `if` reads the matching mask half and the value half (8-byte f64 store). FALSE lanes keep o's
        // sentinel (verified). IEEE f64 exact => SIMD == scalar == reference. Asserts kernel_simd emitted.
        [TestMethod(Timeout = 120000)]
        public async Task Wasm_Simd128_MaskedStoreFloat64MatchesScalar()
        {
            const int N = 1003; const double SENT = -987.25;
            var a = new double[N]; for (int i = 0; i < N; i++) a[i] = Math.Sin(i * 0.05) * 10.0; // ~half > 0
            var reference = new double[N]; for (int i = 0; i < N; i++) reference[i] = a[i] > 0.0 ? a[i] * 2.0 : SENT;
            var scalar = await RunMaskedStoreF64(a, N, SENT, false, false);
            var simd = await RunMaskedStoreF64(a, N, SENT, true, true);
            AssertExactD(scalar, reference, simd, "masked-store-f64");
        }

        private static void Wasm_Simd_MaskedStoreF64Kernel(Index1D i, ArrayView<double> a, ArrayView<double> o) { if (a[i] > 0.0) o[i] = a[i] * 2.0; }

        private static async Task<double[]> RunMaskedStoreF64(double[] a, int N, double sentinel, bool forceSimd, bool requireSimdEmit)
        {
            bool ss = WasmBackend.ForceScalar, sm = WasmBackend.ForceSimd;
            WasmBackend.ForceScalar = !forceSimd; WasmBackend.ForceSimd = forceSimd;
            try
            {
                ...
                WasmBackend.VerboseLogging = false; WasmBackend.LastWasmBinary = null;
                using var acc = await ctx.CreateWasmAcceleratorAsync();
                var k = acc.LoadAutoGroupedStreamKernel<Index1D, ArrayView<double>, ArrayView<double>>(Wasm_Simd_MaskedStoreF64Kernel);
                if (requireSimdEmit && (WasmBackend.LastWasmBinary == null || !ContainsExportName(WasmBackend.LastWasmBinary, "kernel_simd")))
                    throw new Exception("ForceSimd compile did NOT emit a kernel_simd export for the f64 masked store.");
                var oInit = new double[N]; for (int i = 0; i < N; i++) oInit[i] = sentinel;
                using var aB = acc.Allocate1D(a); using var oB = acc.Allocate1D(oInit);
                k((Index1D)N, aB.View, oB.View); await acc.SynchronizeAsync();
                return await oB.CopyToHostAsync<double>();
