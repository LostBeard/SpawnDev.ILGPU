...
| `CopyFrom()` (GPU&rarr;GPU) | OK | **OK on all 3** - ordered after the producing kernel on every backend. WebGPU (native `CopyBufferToBuffer`) / WebGL (TF readback) order it via their queue; Wasm enqueues the copy into the serialized work stream (`CopyFromBuffer` &rarr; `CopyFromBufferOrdered` &rarr; `CopyFromBufferCore`), so a `CopyFrom` from a buffer a kernel just wrote runs AFTER that kernel (fixed local.7; the earlier immediate-host-memcpy stale-read is gone, and the device&rarr;device throw was removed). `CopyFromAsync` is now just optional eager completion, not required for correctness. | fire-and-forget |
...
- **GPU&rarr;CPU readback (and any wait) is the async-only boundary.** GPU&rarr;GPU (`CopyFrom`) and CPU&rarr;GPU (`CopyFromCPU`) uploads + dispatch + `Flush()` are fire-and-forget and sync-safe on browser; prefer them over a readback+reupload. Sync `CopyFrom` is ordered after the producing kernel on **all 6 backends** (Wasm enqueues it into the serialized work stream since local.7), so it does NOT need an `await SynchronizeAsync()`/`CopyFromAsync` first - `CopyFromAsync` is optional eager completion, not a correctness requirement.
