...
**Current version: see [CHANGELOG.md](CHANGELOG.md) — it is the SINGLE SOURCE OF TRUTH for versions/features (this header is NOT kept in lockstep; do not trust a version number written below).** As of 2026-06-23: nuget.org stable = **4.16.2** (wrapper; forks `2.1.0`) = 4.16.0 (CUDA graph capture API + device-local `new T[N>32]` dyn-index fix all 6 backends, forks 2.1.0) PLUS the WebGPU register-attention enablement (WGSL `ShuffleKind`→`subgroupShuffleXor`/Up/Down + module-level `diagnostic(off, subgroup_uniformity)` for subgroup kernels + Wasm shuffle-kind fix; full PMT 4010/0/263; ML verified register attention 8/8 all 6 backends incl WebGPU on Dawn). Previous stable = 4.15.1 (forks 2.0.42). ⚠ Known latent: const-size `new T[]` that PARTIAL-unrolls (D=48/64-class) crashes at the PTX backend - `LowerArrays` leaves a `LoadArrayElementAddress` un-lowered in a split-created block (no production kernel hits it; tracked in memory `reference-ilgpu-const-D-register-scalar-replace-attention`). The rest of this paragraph is HISTORICAL context (rc.7–rc.10 era):
...
`ArrayView<byte>`, `ArrayView<sbyte>`, `ArrayView<short>`, `ArrayView<ushort>`, `ArrayView<Half>` (ILGPU.Half), `ArrayView<BFloat16>` (ILGPU.BFloat16), `ArrayView<Float8E4M3>`/`<Float8E5M2>` (FP8, 1-byte), and the **packed 4-bit** types `ArrayView<Float4E2M1>` (FP4, NVFP4/MXFP4), `ArrayView<QInt4>`, `ArrayView<QUInt4>` supported on all 6 backends.

**Packed 4-bit (v4.14.0+):** `Float4E2M1`/`QInt4`/`QUInt4` are `[PackedBits(4)]` - 2 nibbles/byte, 8 per `u32` word, so an `ArrayView<T>` of N is `ceil(N/2)` device bytes. **Load works on all 6**; **in-kernel STORE works on CUDA/OpenCL/WebGPU/Wasm and is FAIL-LOUD on CPU + WebGL** (the nibble store is an atomic word RMW - WebGL has no atomics, and the CPU managed ref indexer cannot address a sub-byte element; both throw rather than corrupt the word). Radix-sort (keys+pairs, asc+desc) on the 4 store backends. Host I/O is RAW PACKED bytes (no transparent typed pack/unpack) - upload via `((IContiguousArrayView)view.BaseView).AsRawArrayView().CopyFromCPU(packed)`. `<Type>Extensions.RawBitsToFloat(int)` decodes a raw nibble/byte in-kernel on all 6 (FP4/FP8/bf16). Full detail: [Docs/data-type-support.md](Docs/data-type-support.md).
...
| Packed 4-bit (FP4/QInt4/QUInt4)‖ | Load+Store | Load only‖ | Load+Store | Load+Store | Load+Store | Load only‖ |

‖Packed 4-bit (`Float4E2M1`/`QInt4`/`QUInt4`, v4.14.0+): `[PackedBits(4)]`, `ceil(N/2)` device bytes. LOAD on all 6 (nibble decode in-register). In-kernel STORE is an atomic word RMW → works on CUDA/OpenCL/WebGPU/Wasm, **FAIL-LOUD on WebGL** (no atomics) **and CPU** (managed ref indexer can't write a sub-byte element). Radix on the 4 store backends. Detail: [Docs/data-type-support.md](Docs/data-type-support.md).
